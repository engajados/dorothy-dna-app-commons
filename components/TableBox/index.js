    ...
    return (<div className="tablebox">
        {hasHeader && <div className='tbox-header'>
            <InputBaseStyled
                ...
                startAdornment={<Search onClick={() => searchInputRef.current?.focus()} color={theme.palette.secondary.main} />}
        ...
        <div className="tbox-body">
            <table className='tbox-table'>
                <thead>
                    <tr>
                        {columns.map((c) => <th key={c.field}>
                            {c.sortable && <TableSortColumn text={c.label} column={c.field} order={order} direction={direction} onClick={onChangeOrder(c.defaultOrder)} enabled={!canSort} />}
                            {!c.sortable && <>{c.label}</>}
                        </th>)}
                        <th>Ações</th>
                    </tr>
                    ...
                    {data.map(row => (
                        <StyledTableRow key={row.id}>
                            {columns.map(c => {
                                ...
                                if (c.renderer) Element = createElement(c.renderer, { field: c.field, value: row[c.field], row, ...c.rendererProps });

                                return (<td {...c.tdProps} key={c.field}>
                                    {Element}
                                </td>)
                            })}
